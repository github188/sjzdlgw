...
public class HttpClientUtils { 
	...
	public static String sendGetRequest(String reqURL){ 
        ...
        try{ 
        	...
        	LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时连接超时,堆栈轨迹如下", cte); 
        } catch (SocketTimeoutException ste){ 
            LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时读取超时,堆栈轨迹如下", ste); 
        }catch(ClientProtocolException cpe){ 
            //该异常通常是协议错误导致:比如构造HttpGet对象时传入协议不对(将'http'写成'htp')or响应内容不符合HTTP协议要求等 
            LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时协议异常,堆栈轨迹如下", cpe); 
        }catch(ParseException pe){ 
            LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时解析异常,堆栈轨迹如下", pe); 
        }catch(IOException ioe){ 
            //该异常通常是网络原因引起的,如HTTP服务器未启动等 
            LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时网络异常,堆栈轨迹如下", ioe); 
        }catch (Exception e){ 
            LogUtils.getErrorLog().error("请求通信[" + reqURL + "]时偶遇异常,堆栈轨迹如下", e); 
        }finally{ 
            //关闭连接,释放资源 
...
//            httpPost.setEntity(new StringEntity(reqData==null?"":reqData, encodeCharset)); 
    ...
    private static void setParams(Map<String, String> params, HttpPost httpPost, String encodeCharset) throws UnsupportedEncodingException{
    	 if(null != params){ 
    ...
    public static void main(String[] args) { 
        ...
        System.out.println(HttpClientUtils.sendPostSSLRequest("https://123.125.97.66:8085/pay/servlet/CreditPayReqServlte", params, "UTF-8")); 
    }
}