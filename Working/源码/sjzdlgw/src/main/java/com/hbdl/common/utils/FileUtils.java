...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	* @param fileName 保存文件名(如果上传文件大于一个，文件名默认 会添加序号 例如 fileName_1,fileName_2..)
	* @throws Exception
	 */
	public static List<UploadFile> multipleUploadFile(String saveDirectory,String fileName,HttpServletRequest request) throws Exception {
		...
		while (it.hasNext()) {
			...
			// 获取文件名后缀
			String surfix = originalFilename.substring(originalFilename.lastIndexOf(".") + 1);
			String newFileName =  fileName + "." + surfix;
			if(length > 1){
				newFileName = fileName + "_" + index +"." + surfix;
				index ++;
			}
			...
			UploadFile file = new UploadFile(parameterName, saveDirectory, fileName, originalFilename,
					originalFile.getContentType(), surfix);
			files.add(file);
		}
	...
	* @throws Exception
	 ...
	 *            保存根路径
	 * @param fileName
	 *            保存文件名(如果上传文件大于一个，文件名默认 会添加序号 例如 fileName_1,fileName_2..)
	 * @throws Exception
	 */

	/**
	 * 多文件上传(base64方式上传)
	 * @param saveDirectory	文件存储路径
	 * @param newFileName	保存文件名(如果上传文件大于一个，文件名默认 会添加序号 例如 fileName_1,fileName_2..)
	 * @param base64Files	上传文件list<Map>集合 (其中map存储方式{"base64Str" : "base64文件字符串", "fileName":"原始文件名（带后缀）"})
	 * @return
	 * @throws Exception
	 */
	public static List<UploadFile> multipleUploadFileBase64( String saveDirectory, String newFileName, List<Map> base64Files)
			throws Exception {
		List<UploadFile> files = new ArrayList<UploadFile>();

		int index = 1;

		for (Map<String, String> map : base64Files) {
			String fileStr = map.get("base64Str");
			String orgFilename = map.get("fileName");
			String surfix = orgFilename.substring(orgFilename.lastIndexOf(".") + 1);

			String cnewFileName = newFileName;

			if (base64Files.size() > 1) {
				cnewFileName = newFileName + "_" + index;
				index++;
			}
			...
			// dirPath.mkdirs();
			// }
			String filePath = saveDirectory + File.separator + cnewFileName+ "." + surfix;
			Base64Utils.decodeToFile(filePath, fileStr);
			File cfile = new File(filePath);

			UploadFile file = new UploadFile("", saveDirectory, cnewFileName,
					orgFilename, FileTypeUtils.getFileTypeByFile(cfile), surfix);
			files.add(file);
		}
	...
	public static UploadFile singleUploadFileBase64(String saveDirectory,
			String fileName, Map fileDataMap) throws Exception {
		List<Map> list = new ArrayList();
		list.add(fileDataMap);
		List<UploadFile> files = multipleUploadFileBase64(saveDirectory, fileName, list);
		return CollectionUtils.isEmpty(files) ? null : files.get(0);
	}
}
