...
//    $(document).ready(function(){
        function _doExport($this,fileNum) {
            ...
            var url = "${ctxPath}/TunnelAduit/fileIsExist/"+fileNum;
            if (!url.isFinishedTm()) {
                alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
                return false;
            }
//            var url2 = unescape("${ctxPath}/TunnelAduit/download/{sid_pt}").replaceTmById($(event.target).parents(".unitBox:first"));//update by gaoyulong 动态生成url根据路径中{}生成
            var url2="${ctxPath}/TunnelAduit/download/"+fileNum;
...
//            });
            function successfunction(response) {
                ...
                if(response)
                {
                    ...
                    if(obj.isExist=="true")
                    {
                        url = "${ctxPath}/TunnelAduit/download/{sid_pt}".replaceTmById($(event.target).parents(".unitBox:first"));
...
<div id="a_uploadify">
    ...
    <form action="${ctxPath}/TunnelAduit/File" class="pageForm required-validate" enctype="multipart/form-data" method="post" onsubmit="return iframeCallback(this,callback);">
        <input value="${TunnelAduitRecordNum}" name="acceptRecordNum" type="hidden">
    ...
    <div><a id="delete" class="delete" href="${ctxPath}/TunnelAduit/File/delete/{sid_pt}" style="display:none;" callback="dialogAjaxDoneFather" target="ajaxTodo" title="确定要删除吗?" warn="请选择一条记录"><span>删除</span></a></div>
    ...
    <div><a  id="download" href="${ctxPath}/TunnelAduit/fileIsExist/{sid_pt}"  targetType="dialog" warn="请选择一个文档" title="确认要下载吗？" style="display:none;">
...
<!--
<div class="panel">
    <form action="${ctxPath}/TunnelAduit/File?num={sid_pt}"  enctype="multipart/form-data" method="post" >
        <input value="${TunnelAduitRecordNum}" name="acceptRecordNum" type="hidden">
        <p>
            <label>文件名称：</label>
