...
<div class="pageContent">
    <form method="post" action="${ctxPath}/TunnelAduit/add" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
        ...
        <div class="pageFormContent" ${type==1?"":"layoutH='50'"}>
            <div class="a-title" style="display:${type==1?"":"none"}">
                ...
                <div class="a-right">新建计划</div>
            </div>
            <div>
                <p>
                    <label>作业名称：</label>
                    <input name="planName" ${readonly==1?'readonly="readonly"':''}class="required" type="text" size="30"  value="${TunnelAduit.planName}" />
                </p>
                <%--<p>--%>
                    <%--<label>作业状态：</label>--%>
                    <%--<select name="taskStatusTypeID" class="required combo-box">--%>
                        <%--<c:forEach items="${TaskStatusTypeList}" var="ob" varStatus="id">--%>
                            <%--<option value="${ob.taskStatusTypeID}" ${TunnelAduit.taskStatusTypeID eq ob.flawSourceTypeID ? 'selected="selected"':''} >${ob.taskStatusTypeName}</option>--%>
                        <%--</c:forEach>--%>
                    <%--</select>--%>
                <%--</p>--%>
                <%--<p>--%>
                    <%--<label>验收次数：</label>--%>
                    <%--<input ${readonly==1?'readonly="readonly"':''} type="text" size="30" value="${aduitTimes}"/>--%>
                <%--</p>--%>

                <p>
                    <label>验收时间：</label>
                    <input  ${readonly==1?'readonly="readonly"':''} name="planDate" type="text"  value="${TunnelAduit.planDateStr}" class="date" size="30" /><a class="inputDateButton" href="javascript:;">选择</a>
                </p>
