...
$.fn.extend({
	ajaxTodo:function(){
		return this.each(function(){
			...
			$this.click(function(e){
				...
				var url = unescape($this.attr("href")).replaceTmById($(ev.target).parents(".unitBox:first"));
				DWZ.debug(url);
				if (!url.isFinishedTm()) {
	...
	dwzExport: function(){
		function _doExport(e,$this) {
			var ev = e || window.event;
			var $p = $this.attr("targetType") == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
			var $form = $("#pagerForm", $p);
			// var url = $this.attr("href");
			var url = unescape($this.attr("href")).replaceTmById($(ev.target).parents(".unitBox:first"));//update by gaoyulong 动态生成url根据路径中{}生成
		...
		return this.each(function(){
			...
			$this.click(function(e){
				...
				if (title) {
					alertMsg.confirm(title, {
						okCall: function(){_doExport(e,$this);}
					});
				} else {_doExport(e,$this);}
