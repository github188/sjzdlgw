...
/*动态计算顶部搜索框高度*/
function aaHeight($this) {
	var aSearchBar = $this.closest('.pageContent').prev('.pageHeader').find('.a-searchBar');
	if ( aSearchBar.length >= 1 ) {
		return aSearchBar.height();
	}
	else {
		return 0;
	}
	...
	$.fn.extend({
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				var iLayoutH = parseInt($this.attr("layoutH")) + aaHeight($this);
