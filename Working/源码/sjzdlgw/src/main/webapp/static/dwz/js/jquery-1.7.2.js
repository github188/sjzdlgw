...
jQuery.fn.extend({
	...
	append: function() {
		...
		$("[controlID]").each(function (elem) {
			...
			// while(controlIDs==null) sleep(100);
			var i=0;
			...
			if(i>=controlIDs.length){
				console.log(this.getAttribute("controlTest"));
		...
		// var num = 4000;
		// $("[controlTest='"+num+"']").remove();
		// $("[controlTest='4001']").remove();

		// 设置查看页面为只读属性
		setTimeout(function(){
			if($("#test").val()==1){
				if($(".pageForm td").length){
                    $("td input").attr("readonly","true");
                    $("td select").attr("disabled","disabled");
                    $("td textarea").attr("readonly","true");
                    $(".combox .select").removeAttr("id");
				}else if($(".pageForm p").length){
                    $("p input").attr("readonly","true");
                    $("p select").attr("disabled","disabled");
                    $("dl textarea").attr("readonly","true");
				}
