...
var navTab = {
	...
	reload: function(url, options){
		...
		if ($panel){
			...
			if (url) {
				if ($tab.hasClass("external")) {
					navTab.openExternal(op.id,url, $panel);
				} else {
	...
	openExternal:function(id,url, $panel){
		var ih = navTab._panelBox.height();
		$panel.html(DWZ.frag["externalFrag"].replaceAll("{id}", id).replaceAll("{url}", url).replaceAll("{height}", ih+"px"));
	},
	/**
	 * 
	 * @param {Object} tabid
